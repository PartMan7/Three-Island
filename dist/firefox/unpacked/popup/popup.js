...
opts.forEach(({ id, states, stateLabels, set, std }) => {
	...
	element.addEventListener('click', e => {
		...
		console.log('donotpush', { browser, window }); // donotpush
		browser.storage.sync.set({ [set]: nextState }).catch(console.error);
	});
});

window.addEventListener('DOMContentLoaded', () => {
	opts.forEach(({ id, states, stateLabels, set, std }) => {
		browser.storage.sync.get(set).then(fetched => {
			let val = fetched[set];
			if (!states.includes(val)) {
				val = std;
				browser.storage.sync.set({ [set]: val }).catch(console.error);
			}
