...
async function build() {
  const manifest = {
    ...
    manifest_version: 0,
    description:
      'A {BROWSER} extension for Pokémon Showdown that offers a seamless Poképaste experience.',
  ...
  firefoxManifest.description = firefoxManifest.description.replace(
    /{BROWSER}/g,
    'Firefox',
  );
  ...
  chromeManifest.description = chromeManifest.description.replace(
    /{BROWSER}/g,
    'Chrome',
  );
  ...
  await fs.copy(
    ...
    path.join(firefoxPath, 'unpacked', 'icons'),
  );
  await fs.copy(
    path.join(__dirname, 'icons'),
    path.join(chromePath, 'unpacked', 'icons'),
  );

  await fs.copy(
    path.join(__dirname, 'popup'),
    path.join(firefoxPath, 'unpacked', 'popup'),
  );
  await fs.copy(
    path.join(__dirname, 'popup'),
    path.join(chromePath, 'unpacked', 'popup'),
  );

  const popupJS = await fs.readFile(
    path.join(chromePath, 'unpacked', 'popup', 'popup.js'),
    'utf8',
  );
  await fs.writeFile(
    path.join(chromePath, 'unpacked', 'popup', 'popup.js'),
    popupJS.replace(/browser/g, 'chrome'),
  );

  await fs.writeFile(
    path.join(firefoxPath, 'unpacked', 'manifest.json'),
    JSON.stringify(firefoxManifest, null, '\t') + '\n',
  );
  await fs.writeFile(
    path.join(chromePath, 'unpacked', 'manifest.json'),
    JSON.stringify(chromeManifest, null, '\t') + '\n',
  );

  await fs.copy(
    path.join(__dirname, 'src', 'syringe.js'),
    path.join(chromePath, 'unpacked', 'syringe.js'),
  );

  const header = (
    await fs.readFile(path.join(__dirname, 'src', 'header.js'), 'utf8')
  ).replace(/{VERSION}/g, package.version);
  const PS_HELPERS = (
    await fs.readFile(path.join(__dirname, 'src', 'ps-helpers.js'), 'utf8')
  )
    .split('\n')
    .map((line) => (line ? `\t${line}` : line))
    .join('\n');
  const CSS = (
    await fs.readFile(path.join(__dirname, 'src', 'showdown.css'), 'utf8')
  )
    .split('\n')
    .map((line) => (line ? `\t${line}` : line))
    .join('\n');
  const ThreeIsland = (
    await fs.readFile(path.join(__dirname, 'src', 'three-island.js'), 'utf8')
  )
    .replace(/'{PS_HELPERS}';/g, PS_HELPERS)
    .replace(/{CSS}/g, CSS);

  const firefoxScript = await fs.readFile(
    path.join(__dirname, 'src', 'firefox.js'),
    'utf8',
  );
  const chromeScript = await fs.readFile(
    path.join(__dirname, 'src', 'chrome.js'),
    'utf8',
  );
  const monkeyScript = await fs.readFile(
    path.join(__dirname, 'src', 'monkey.js'),
    'utf8',
  );
  ...
  await fs.writeFile(
    path.join(firefoxPath, 'unpacked', 'three-island.js'),
    firefoxScript.replace(/{THREE-ISLAND}/g, indentedThreeIsland),
  );
  await fs.writeFile(
    path.join(chromePath, 'unpacked', 'three-island.js'),
    chromeScript.replace(/{THREE-ISLAND}/g, indentedThreeIsland),
  );

  await fs.writeFile(
    path.join(scriptPath, 'greasemonkey.js'),
    `${header}\n${monkeyScript}\n${ThreeIsland}`,
  );
  await fs.writeFile(
    path.join(scriptPath, 'tampermonkey.js'),
    `${header}\n${monkeyScript}\n${ThreeIsland}`,
  );
  ...
  // Create archives
  await zip.folder(
    path.join(firefoxPath, 'unpacked'),
    path.join(firefoxPath, 'three_island-latest.xpi'),
  );
  await zip.folder(
    path.join(chromePath, 'unpacked'),
    path.join(chromePath, 'three_island-latest.zip'),
  );

  await fs.copy(
    path.join(firefoxPath, 'three_island-latest.xpi'),
    path.join(firefoxPath, `three_island-${package.version}.xpi`),
  );
  await fs.copy(
    path.join(chromePath, 'three_island-latest.zip'),
    path.join(chromePath, `three_island-${package.version}.zip`),
  );
}
